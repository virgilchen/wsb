 ...
 * 
 * 
 * tesseract 1259.png result digits
 */
public class ImagePreProcess {
    ...
    private static String BASE_PATH = "D://temp//ocr//" ;
    ...
    public static BufferedImage removeBackgroud(String picFile) throws Exception {  
        ...
        //BufferedImage img = grayImage(ImageIO.read(new File(picFile)));  
    	
        MyImgFilter flt = new MyImgFilter(ImageIO.read(new File(picFile)));
		flt.changeGrey();
		flt.getGrey();
		flt.getBrighten();
		BufferedImage bi =  flt.getProcessedImg();
		...
		if (img != null) {
			return img.getSubimage(2, 2, img.getWidth() - 3, img.getHeight() - 3);
		}
		
        img = img.getSubimage(2, 2, img.getWidth() - 3, img.getHeight() - 3);  
        ...
        for (int i = 0; i < 5; i++) {  
            ...
            for (int x = (int) (1 + i * subWidth); x < (i + 1) * subWidth  
                    && x < width - 1; ++x) {  
                for (int y = 0; y < height; ++y) {  
                    if (img.getRGB(x, y) > colorMax/1.2) {  
    ...
    public static Map<BufferedImage, String> loadTrainData() throws Exception {  
        if (trainMap == null) {  
            ...
            File dir = new File(BASE_PATH + "train");  
    ...
    public static String getAllOcr(String file) throws Exception {  
        ...
        ImageIO.write(img, "JPG", new File(BASE_PATH + "result//" + result + ".jpg"));  
        return result;  
    }  
    ...
    public static void trainData() throws Exception {  
        File dir = new File(BASE_PATH + "img");  
        File[] files = dir.listFiles();  
        for (File file : files) {  
            BufferedImage img = removeBackgroud(BASE_PATH + "img//" + file.getName());  
            List<BufferedImage> listImg = splitImage(img);  
    
            for (int j = 0; j < listImg.size(); ++j) {  
                ImageIO.write(listImg.get(j), "JPG", new File(BASE_PATH + "train//"  
    ...
    public static void main(String[] args) throws Exception {  
        ...
        String text = getAllOcr(BASE_PATH + "img//image.jpg");  
